...
type PluginConfig struct {
	...
	// 指定了 plugin config 在 segment/master host 分发后的路径. 并不是用户在调用 gpbackup 时通过 plugin config 参数传递来的路径.
	// 参见 'gpbackup 对 plugin config 的处理' 了解背景.
...
// 参见 'gpbackup 对 plugin config 的处理' 了解该函数使用场景.
// 对于一个机器上会存在多个 primary segment 的情况, 这里 contentIDForSegmentOnHost 是随机选择的一个, 意义不大.
